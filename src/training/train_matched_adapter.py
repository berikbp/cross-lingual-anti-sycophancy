...
OUTPUT_ROOT = Path("outputs/adapters")
REPORT_ROOT = Path("reports/training_runs")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--condition",
        required=True,
        choices=["control", "anti_sycophancy"],
    )
    parser.add_argument(
        "--config",
        type=Path,
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Train on two records for two optimizer steps.",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
...
def main() -> None:
    ...
    output_name = (
        f"{condition}_dry_run"
        if arguments.dry_run
        else condition
    )
    output_dir = OUTPUT_ROOT / output_name
    report_dir = REPORT_ROOT / output_name
    ...
    if arguments.dry_run:
        ...
        validation_records = validation_records[:2]

    expected_train = 2 if arguments.dry_run else 1000
    expected_validation = 2 if arguments.dry_run else 100
