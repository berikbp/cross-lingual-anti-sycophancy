...
in `v1.1.1`, repackaged with normalized Zenodo metadata in `v1.1.2`, and
completed with reviewed adapter cards and prose corrections in `v1.1.3`;
...
| Adapter weights and raw generations lack public download locations | High | reproducibility guide and inventory | A fresh clone cannot run all hash checks | Publish external artifacts with URLs and checksums; distinguish clone-only from external-artifact checks | Closed in `v1.1.1` and republished through `v1.1.3`: source, raw-result, and Apache-2.0 adapter assets are public with separate checksum files |
...
| No corrected GitHub Release exists | Medium | GitHub release metadata | Zenodo cannot archive the corrected release workflow | Create an immutable corrected GitHub Release after all blocking corrections | Closed: `v1.1.0` preserved the audited release; `v1.1.1` added public assets; `v1.1.2` normalized Zenodo metadata; `v1.1.3` completes publication QA |
...
`v1.1.3` GitHub release is the source for Zenodo deposits. Zenodo DOI assignment
