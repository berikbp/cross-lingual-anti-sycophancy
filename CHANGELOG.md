...
## 1.1.3 — publication quality audit

- Corrected the paper's description of answer-position balance: the 400-item
  master pool is exactly balanced, while the frozen 300/100 split retains its
  sampled distributions.
- Replaced autogenerated placeholder adapter READMEs with concise, condition-
  specific model cards.
- Updated the adapter bundler to package the reviewed cards without modifying
  the frozen weights or adapter configurations.
- Removed stale draft wording from the repository map and release report.
- Scientific data, model weights, raw results, analyses, and conclusions are
  unchanged.
